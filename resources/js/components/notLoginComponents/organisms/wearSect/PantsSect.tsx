...
export const PantsSect: FC<Props> = memo((props) => {
    const { notLoginUser, setNotLoginUser } = useNotLoginUser();
    const { onClickPants, defaultGender, setDataPants, setPantsArray, dataPants, pantsArray, getActiveIndexPants, getPants, userPants, defaultPants } = props;
    ...
    useEffect(() => {
        if (userPants[0]) {
            if (count > 0) {
                showMessage({ title: `${userPants[0].count}件見つかりました`, status: "success" });
            }
        }
    }, [count])
    ...
    return (
        pantsArray.length ? (
            <>
                ...
                >
                    {pantsArray.map((wear) => (
                        <SwiperSlide onClick={onClickPants} className="wearLi" key={wear.id}  >
        ...
        ) : (defaultPants ? (defaultUrl ? (
            <>
                <div onClick={onClickPants} style={{ width: "100%", height: "170px", margin: "auto" }}>
